...
use windows::Win32::{
    Foundation::*,
    System::{Diagnostics::Debug::*, ProcessStatus::EnumProcessModules, Threading::*},
};
...
async fn init_process(window: Window) {
    ...
    if processes.len() > 1 {
        println!("Found more than 1 process of BlackOps.exe!");
    }
            ...
            Ok(process_handle) => {
                ...
                loop {
                    ...
                    if read_success.is_ok() && bytes_read == std::mem::size_of::<u32>() {
                        ...
                        let read_final_value_success = ReadProcessMemory(
                            process_handle,
                        ...
                        if read_final_value_success.is_ok()
                            && bytes_read == std::mem::size_of::<u32>()
                        {
                            ...
                            println!("Failed to read game timer value.");
                        }
                    } else {
